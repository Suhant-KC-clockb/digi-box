...
const Tab = ({ children, tab, handleSetSelected, selected }: any) => {
  return (
    <button
      ...
      className={`flex items-center gap-1 rounded-full px-3 py-1.5 text-md transition-colors ${
        selected === tab ? "bg-neutral-800 text-neutral-100" : ""
      }`}
    >
      <span>{children}</span>
      <FiChevronDown
...
const Content = ({ selected, dir }: any) => {
  return (
    <motion.div
      ...
      className="absolute left-0 top-[calc(100%_+_24px)] w-96 rounded-lg border bg-gradient-to-b bg-card p-4"
    >
      <Bridge />
